"""
App de escritorio
toma los datos del dolar de la web
...
def convertirDatos(dato):
    try:
        ...
        messagebox.showerror("Error de cantidad","Debe ingresar numeros enteros en la cantidad de empanadas")
        dato="Error"
        borrarEntrada()
    return dato
    ...
    if not nombre:
        ...
        if total==0:
            borrarEntrada()
             ...
             messagebox.showinfo("Exito","Pedido realizado")
             borrarEntrada()
        else:
            borrarEntrada()
...
def dolarweb():
    ...
    dolar= dolar.replace(",",".")
    valor= float(dolar) #1 dolar - 1 empanada
    return valor
